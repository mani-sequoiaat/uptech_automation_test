...
// <<< NEW: Import your custom DB client utility >>>
// The path goes up two directories to the root, then into utils
const { getDbClient, closeDbClient } = require('../../utils/dbClient.js'); 

// <<< MODIFIED: Removed dbConfig from this import >>>
const {
  sftpConfig,
  sftpRemoteDir,
  outputDir,
...
// const locationCodes = require('./data/locationCodes'); // No longer used
const usStates      = require('./data/usStates');

const sftp = new Client();
// <<< REMOVED: const pool = new Pool(dbConfig); >>>

// --- Sequence Helpers ---
// ... (all sequence helper functions remain unchanged) ...
...
// ... (Index mappings and output directories remain unchanged) ...
const defleetIdx = {
...
// ... (parseCsvRecords function remains unchanged) ...
...
// <<< MODIFIED: This function now uses your getDbClient utility >>>
async function fetchLocationData() {
  const query = `
    SELECT 
        ...
        addr.zip,
        st.state_code
    FROM "FleetAgency".location AS loc
    LEFT JOIN "MasterData".address AS addr 
        ON loc.address_id = addr.id
    LEFT JOIN "MasterData".state AS st
        ON addr.state_id = st.id
    ORDER BY loc.id DESC
    LIMIT 100;
  `;
  try {
    // Get the shared, connected client from your utility
    const client = await getDbClient(); 
    // Use the client to run the query
    const { rows } = await client.query(query); 
    
    console.log(`  Successfully fetched ${rows.length} location records from DB.`);
    return rows;
  } catch (err) {
    console.error('! Error fetching location data from database:');
    console.error(err);
    return null; // Handle this error in the calling function
  }
}

// ... (generateSFleetData function remains unchanged) ...
function generateSFleetData(count, locationData) {
  const arr = [];
  const locationCount = locationData.length;

  for (let i = 1; i <= count; i++) {
    const location = locationData[(i - 1) % locationCount];
    ...
    arr.push({
      ...
      location_1:     location.address_1,
      location_2:     location.address_2 || `Block ${String(i).padStart(6,'0')}`, 
      city:           location.city,
...
// ... (SFTP helper functions remain unchanged) ...
...
// ... (generateAndMerge function logic remains the same, only 'finally' block changed) ...
async function generateAndMerge(count, fmt) {
  let locationData;
  try {
    console.log(' Step 0.5: Fetching location data from DB');
    locationData = await fetchLocationData();

    if (!locationData || locationData.length === 0) {
      console.error('! Aborting: Could not fetch location data. Check DB connection and query.');
      process.exit(1);
    }

    console.log(' Step 1: Generate synthetic fleet data');
    const sfleet_data = generateSFleetData(count, locationData);
    ...
    if (yestLocal && fs.existsSync(yestLocal)) {
      ...
      raw.shift();
      if (raw.length >= 2) raw.splice(-2, 2);
      oldRecords = raw;
    ...
    saveJson(fleetOutputDir, 'fleet_records', ts, infleetRecords.map(r => ({
      ...
      license_plate_state:  r.license_distance
    })));
    ...
    // <<< MODIFIED: Use your utility to close the DB connection >>>
    await closeDbClient();
    console.log('Database connection closed.');
  }
}

// ... (CLI Entrypoint remains unchanged) ...
